...
export class BasededatosService {
  ...
  registroRol2: string = "INSERT or IGNORE INTO rol(id_rol,nombre_r) VALUES (1,'Conductor');";
  registroRol: string = "INSERT or IGNORE INTO rol(id_rol,nombre_r) VALUES (2,'Usuario');";
  listaRol = new BehaviorSubject([]);
  //tabla usuarios
  tUsuario: string = "CREATE TABLE IF NOT EXISTS usuario(correo VARCHAR(150) PRIMARY KEY, nombre VARCHAR(40), apellido VARCHAR(40) ,telefono INTEGER, contrasennia VARCHAR(40) NOT NULL,bonificacion INTEGER,foto BLOB, tR_idRol INTEGER, FOREIGN KEY(tR_idRol) REFERENCES Rol(id_rol));";
  listaUsuarios = new BehaviorSubject([]);
  //tabla auto
  tAuto: string = "CREATE TABLE IF NOT EXISTS auto(patente VARCHAR(8) PRIMARY KEY,modelo VARCHAR(35),marca VARCHAR(35),annio INTEGER, tU_correo VARCHAR(150), FOREIGN KEY (tU_correo) REFERENCES Usuario(correo));";
  ...
  */
  ...
  buscarPerfil(usuario) {
    let data = [usuario]
    //retorno la ejecución del select
    return this.database.executeSql("SELECT correo,nombre,apellido,nombre||' '||apellido as completo, telefono,foto FROM usuario where correo = ?", data).then(res => {
  ...
  buscarPerfilC(usuario2) {
    let data = [usuario2]
    //retorno la ejecución del select
    return this.database.executeSql("SELECT correo,nombre,apellido,nombre||' '||apellido as completo,auto.marca ||' '||auto.modelo as Vehiculo,telefono,foto FROM usuario INNER JOIN auto on usuario.correo = auto.tU_correo  where usuario.correo = ?", data).then(res => {
      //creo mi lista de objetos de noticias vacio
      let items3: PerfilC[] = [];
      //si cuento mas de 0 filas en el resultSet entonces agrego los registros al items
      if (res.rows.length > 0) {
  ...
  */
  ...
  insertarA(patente,modelo,marca,annio,usuario){
    let data= [patente,modelo,marca,annio,usuario]
    return this.database.executeSql('INSERT INTO auto(patente,modelo,marca,annio,tU_correo) VALUES (?,?,?,?,?);', data).then(res => {
      this.buscarAutos();
    });
